 ...
 * @method     ChildUsersQuery orderByWalletId($order = Criteria::ASC) Order by the wallet_id column
 ...
 * @method     ChildUsersQuery groupByWalletId() Group by the wallet_id column
 ...
 *
 * @method     ChildUsersQuery leftJoinWallets($relationAlias = null) Adds a LEFT JOIN clause to the query using the Wallets relation
 * @method     ChildUsersQuery rightJoinWallets($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Wallets relation
 * @method     ChildUsersQuery innerJoinWallets($relationAlias = null) Adds a INNER JOIN clause to the query using the Wallets relation
 *
 * @method     ChildUsersQuery joinWithWallets($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Wallets relation
 *
 * @method     ChildUsersQuery leftJoinWithWallets() Adds a LEFT JOIN clause and with to the query using the Wallets relation
 * @method     ChildUsersQuery rightJoinWithWallets() Adds a RIGHT JOIN clause and with to the query using the Wallets relation
 * @method     ChildUsersQuery innerJoinWithWallets() Adds a INNER JOIN clause and with to the query using the Wallets relation
 *
 * @method     ChildUsersQuery leftJoinTokens($relationAlias = null) Adds a LEFT JOIN clause to the query using the Tokens relation
 * @method     ChildUsersQuery rightJoinTokens($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Tokens relation
 * @method     ChildUsersQuery innerJoinTokens($relationAlias = null) Adds a INNER JOIN clause to the query using the Tokens relation
 *
 * @method     ChildUsersQuery joinWithTokens($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Tokens relation
 *
 * @method     ChildUsersQuery leftJoinWithTokens() Adds a LEFT JOIN clause and with to the query using the Tokens relation
 * @method     ChildUsersQuery rightJoinWithTokens() Adds a RIGHT JOIN clause and with to the query using the Tokens relation
 * @method     ChildUsersQuery innerJoinWithTokens() Adds a INNER JOIN clause and with to the query using the Tokens relation
 *
 * @method     \WalletsQuery|\TokensQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildUsers findOneByWalletId(int $wallet_id) Return the first ChildUsers filtered by the wallet_id column
 ...
 * @method     ChildUsers requireOneByWalletId(int $wallet_id) Return the first ChildUsers filtered by the wallet_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUsers[]|ObjectCollection findByWalletId(int $wallet_id) Return ChildUsers objects filtered by the wallet_id column
...
abstract class UsersQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'exchange', $modelName = '\\Users', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, login, email, firstname, surname, password, wallet_id, status FROM users WHERE id = :p0';
        try {
    ...
    public function filterByWalletId($walletId = null, $comparison = null)
    {
        if (is_array($walletId)) {
            ...
            if (isset($walletId['min'])) {
                $this->addUsingAlias(UsersTableMap::COL_WALLET_ID, $walletId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($walletId['max'])) {
                $this->addUsingAlias(UsersTableMap::COL_WALLET_ID, $walletId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsersTableMap::COL_WALLET_ID, $walletId, $comparison);
    }
    ...
    /**
     * Filter the query by a related \Wallets object
     *
     * @param \Wallets|ObjectCollection $wallets The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
     ...
     */
    public function filterByWallets($wallets, $comparison = null)
    {
        if ($wallets instanceof \Wallets) {
            return $this
                ->addUsingAlias(UsersTableMap::COL_WALLET_ID, $wallets->getId(), $comparison);
        } elseif ($wallets instanceof ObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(UsersTableMap::COL_WALLET_ID, $wallets->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByWallets() only accepts arguments of type \Wallets or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the Wallets relation
     *
     ...
     * @return $this|ChildUsersQuery The current query, for fluid interface
     */
    public function joinWallets($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('Wallets');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'Wallets');
        }
    ...
    /**
     * Use the Wallets relation Wallets object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \WalletsQuery A secondary query class using the current class as primary query
     */
    public function useWalletsQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinWallets($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Wallets', '\WalletsQuery');
    }

    /**
     * Filter the query by a related \Tokens object
     *
     * @param \Tokens|ObjectCollection $tokens the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildUsersQuery The current query, for fluid interface
     */
    public function filterByTokens($tokens, $comparison = null)
    {
        if ($tokens instanceof \Tokens) {
            return $this
                ->addUsingAlias(UsersTableMap::COL_ID, $tokens->getUserId(), $comparison);
        } elseif ($tokens instanceof ObjectCollection) {
            return $this
                ->useTokensQuery()
                ->filterByPrimaryKeys($tokens->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByTokens() only accepts arguments of type \Tokens or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the Tokens relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return $this|ChildUsersQuery The current query, for fluid interface
     */
    public function joinTokens($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('Tokens');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'Tokens');
        }
    ...
    /**
     * Use the Tokens relation Tokens object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \TokensQuery A secondary query class using the current class as primary query
     */
    public function useTokensQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinTokens($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Tokens', '\TokensQuery');
    }
